# 🏠 Simple Homelab Setup# 🏠 Simple Homelab Setup# 🏠 Simple Homelab Setup# 🏠 Simple Homelab Setup# 🏠 Simple Homelab Setup# 🏠 Simple Homelab Setup
...
- ✅ Installs Docker and Docker Compose**Maintainer:** J35867U

- ✅ Downloads the entire homelab repository

- ✅ Deploys all services in the correct orderDeploy everything with one command on Proxmox:
...
homelab/wget -O deploy.sh https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-deploy-homelab.sh && chmod +x deploy.sh && ./deploy.sh**Last Updated:** 2025-10-16
...
│   └── media/          # Media management & streaming (Jellyfin, Servarr)## 🚀 Quick Start
...
```- ✅ Installs Docker and Docker Compose**Maintainer:** J35867U  **Maintainer:** J35867U
...
### Download ClientsClean, organized Docker homelab with manual control over service groups.
...
- **Radarr** - Movie managementhomelab/wget -O deploy.sh https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-deploy-homelab.sh && chmod +x deploy.sh && ./deploy.sh**Last Updated:** 2025-10-16
...
- **Vaultwarden** - Password manager│   └── media/          # Media management & streaming (Jellyfin, Servarr)## 🚀 Quick Start (Recommended)
...
│   Core Stack    │    │  Download Stack │    │   Media Stack   │```- ✅ Installs Docker and Docker Compose**Last Updated:** 2025-10-16
...
│ (Optional)      │                          └─────────────────┘### Core Infrastructure- ✅ Deploys all services in the correct order**Deploy everything with one command on Proxmox:**
...
- **Prowlarr**: http://localhost:9696 - Indexer management### Media Management```bash
...
### Optional Infrastructure (Proxmox only)

- **Nginx Proxy Manager**: http://192.168.1.201:81 - Reverse proxy- **Radarr** - Movie managementhomelab/wget -O deploy.sh https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-deploy-homelab.sh && chmod +x deploy.sh && ./deploy.sh**Deploy everything with one command on Proxmox:**
...
```bash### Optional LXC Services (Proxmox)│   ├── core/           # VPN & networking (Gluetun, FlareSolverr)```
...
cd containers/core- **Vaultwarden** - Password manager│   └── media/          # Media management & streaming (Jellyfin, Servarr)## 🚀 Quick Start (Recommended)
...
cd ../media## 🏗️ Architecture├── lxc/                # LXC container servicesThis automatically:
...
### 3. One-Command Deploy```└── docs/               # Documentation
...
```│   Core Stack    │    │  Download Stack │    │   Media Stack   │```- ✅ Installs Docker and Docker Compose## Structure
...
```bash│ (Optional)      │                          └─────────────────┘### Core Infrastructure- ✅ Deploys all services in the correct order**Deploy everything with one command on Proxmox:**
...
## 🌐 Proxmox Integration│  • Pi-hole      │



Optimized for Proxmox VE:│  • Tailscale    │- **FlareSolverr** - Cloudflare bypass service- ✅ Provides access URLs when complete



- **LXC Services** - Infrastructure services in lightweight containers│  • Vaultwarden  │
...
wget https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-nuclear-cleanup.sh### Download Clients```bash

chmod +x proxmox-nuclear-cleanup.sh

./proxmox-nuclear-cleanup.sh## 🌐 Service Access
...
- **RAM**: Minimum 4GB, 8GB+ recommended- **Jellyfin**: http://localhost:8096 - Media server- **NZBGet** - Efficient Usenet downloader## 📁 Structure

- **Storage**: 100GB minimum, 500GB+ recommended for media

- **Network**: Reliable internet connection- **Sonarr**: http://localhost:8989 - TV show automation
...
```bash- **Prowlarr**: http://localhost:9696 - Indexer management### Media Management```bash

cp env.example .env

```- **JellyStat**: http://localhost:3000 - Analytics

Edit `.env` with your PUID/PGID, timezone, VPN credentials, and API keys.

- **Jellyfin** - Media server and streaming

### VPN Configuration

Configure `containers/core/wg0.conf` with your VPN provider's WireGuard settings.### Download Services



### Media Layout- **qBittorrent**: http://localhost:8080 - Torrent client- **Prowlarr** - Indexer manager```

```

/data/media/- **NZBGet**: http://localhost:6789 - Usenet client

├── movies/     # Movie files

├── shows/      # TV show files- **Sonarr** - TV series management

├── music/      # Music library

└── downloads/  # Download staging### Optional LXC Infrastructure (Proxmox only)

```

- **Nginx Proxy Manager**: http://192.168.1.201:81 - Reverse proxy- **Radarr** - Movie managementhomelab/wget -O deploy.sh https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-deploy-homelab.sh && chmod +x deploy.sh && ./deploy.shwget -O deploy.sh https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-deploy-homelab.sh && chmod +x deploy.sh && ./deploy.sh```
...
1. **Port conflicts** - Check if ports are already in use- **Ntfy**: http://192.168.1.203:80 - Push notifications- **Bazarr** - Subtitle management

2. **Permission errors** - Verify PUID/PGID settings

3. **VPN connectivity** - Check Gluetun logs for VPN status- **Vaultwarden**: http://192.168.1.205:80 - Password manager
...
# Check specific service### 1. Setup Environment
...
- **[Directory Structure](docs/Directory_Tree_v1.0.txt)** - File organization- **Pi-hole** - Network-wide ad blocking│   ├── downloads/      # Download clients (qBittorrent, NZBGet)



## 🔄 Backup & Recovery### 2. Deploy Step by Step



### Automated Backups```bash- **Tailscale** - Secure remote access

- **Restic** - Encrypted, incremental backups

- **Daily summaries** - Backup status reports# Start core services first (VPN)

- **Documentation archiving** - Config and documentation backups

cd containers/core- **Vaultwarden** - Password manager│   └── media/          # Media management & streaming (Jellyfin, Servarr)```homelab/

### Disaster Recovery

1. **Fresh Installation**: Clone repo on new systemdocker-compose up -d

2. **Configuration**: Copy backed-up `.env` and `wg0.conf`

3. **Deployment**: Run `./setup/deploy-all.sh`- **Ntfy** - Push notifications
...
- **Discussions**: GitHub Discussions for questions and community supportdocker-compose up -d
...
- **Servarr** applications./deploy-all.sh

- **Jellyfin** media server

- **Gluetun** VPN client```│   Core Stack    │    │  Download Stack │    │   Media Stack   │```- ✅ Installs Docker and Docker Compose├── containers/
...
```bash│ (Optional)      │                          └─────────────────┘### **Core Infrastructure**- ✅ Deploys all services in the correct orderThis automatically:│   ├── core/           # VPN & networking (Gluetun, FlareSolverr)
...
This homelab is optimized for Proxmox VE:│  • Tailscale    │- **FlareSolverr** - Cloudflare bypass service- ✅ Provides access URLs when complete
...
wget https://raw.githubusercontent.com/J358763H/homelab/main/proxmox-nuclear-cleanup.sh### **Download Clients**- ✅ Installs Docker and Docker Compose│   ├── downloads/      # Download clients (qBittorrent, NZBGet)
...
```bash- **Prowlarr**: http://localhost:9696 - Indexer management### **Media Management**- ✅ Downloads the entire homelab repository│   └── media/          # Media management & streaming (Jellyfin, Servarr)
...
### Media Organization- **qBittorrent**: http://localhost:8080 - Torrent client- **Prowlarr** - Indexer manager```
...
- **Gluetun** VPN client```│   Core Stack    │    │  Download Stack │    │   Media Stack   │```## Quick Start
