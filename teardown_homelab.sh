...
echo "Tearing down Homelab-SHV services..."
...
sudo crontab -l 2>/dev/null | grep -v "# Homelab-SHV Cron Jobs" | grep -v "restic_backup_with_alerts.sh" | grep -v "hdd_health_check.sh" | grep -v "daily_backup_summary.sh" | grep -v "maintenance_dashboard.sh" | grep -v "weekly_system_health.sh" | grep -v "Homelab_Documentation_Archiver.sh" | sudo crontab -
