...
# Enhanced with Proxmox LXC specific validation
...
generate_summary() {
    ...
    if [ $FAILED_CHECKS -eq 0 ]; then
        ...
        if [ "$PROXMOX_ENV" = true ]; then
            echo -e "  • Enable TUN device: ${WHITE}echo 'lxc.cgroup2.devices.allow: c 10:200 rwm' >> /etc/pve/lxc/CTID.conf${NC}"
            echo -e "  • Enable nesting: ${WHITE}echo 'lxc.apparmor.profile: unconfined' >> /etc/pve/lxc/CTID.conf${NC}"
        fi
        return 1
    fi
}

# 🏥 Proxmox LXC Critical Validation
check_proxmox_requirements() {
    ...
    if [[ "$kernel_version" == *"pve"* ]]; then
        print_check "PASS" "Proxmox kernel detected: $kernel_version"
    else
        print_check "WARN" "Non-Proxmox kernel: $kernel_version" "May cause compatibility issues"
    fi

    # Check if we're in an LXC container
    if [ -f "/proc/1/cgroup" ] && grep -q "lxc" /proc/1/cgroup 2>/dev/null; then
        print_check "INFO" "Running inside LXC container"

        # Check TUN device access (critical for Gluetun/VPN)
        if [ -c "/dev/net/tun" ]; then
            print_check "PASS" "TUN device available for VPN containers"
            has_tun=true
        else
            print_check "FAIL" "TUN device missing - VPN containers will fail" "Run: mknod /dev/net/tun c 10 200 && chmod 666 /dev/net/tun"
        fi

        # Check for privileged/nesting capabilities
        if grep -q "lxc.apparmor.profile.*unconfined" /proc/mounts 2>/dev/null || [ "$(cat /proc/sys/kernel/unprivileged_userns_clone 2>/dev/null)" = "1" ]; then
            print_check "PASS" "Container has sufficient privileges"
            has_nesting=true
        else
            print_check "WARN" "Limited container privileges" "May need privileged=1 or nesting=1 in LXC config"
        fi
    else
        print_check "INFO" "Running on Proxmox host"
        has_tun=true
        has_nesting=true
    fi

    # Check cgroup v2 compatibility
    if [ -d "/sys/fs/cgroup/unified" ] || grep -q "cgroup2" /proc/filesystems 2>/dev/null; then
        print_check "PASS" "cgroup v2 support available"
    else
        print_check "WARN" "cgroup v2 limited" "Modern container features may not work"
    fi

    # Check for common kernel modules needed
    local modules=("ip_tables" "iptable_nat" "xt_MASQUERADE" "bridge" "veth")
    ...
    if [ ${#missing_modules[@]} -eq 0 ]; then
        print_check "PASS" "Required kernel modules available"
    else
        print_check "WARN" "Missing kernel modules: ${missing_modules[*]}" "May affect networking/firewall"
    fi

    # DNS loop detection (critical for Pi-hole)
    if [ -f "/etc/resolv.conf" ]; then
        local dns_servers=$(grep "nameserver" /etc/resolv.conf | awk '{print $2}' | grep -v "127\|::1")
        if [ -n "$dns_servers" ]; then
            print_check "PASS" "External DNS servers configured" "Using: $(echo $dns_servers | tr '\n' ' ')"
        else
            print_check "WARN" "DNS may cause bootstrap loop with Pi-hole" "Ensure upstream DNS is configured"
        fi
    fi
    ...
    if [ "$available_space" -gt 5242880 ]; then  # 5GB in KB
        print_check "PASS" "Sufficient disk space available"
    else
        print_check "WARN" "Limited disk space: $(( available_space / 1024 ))MB" "Consider cleanup before deployment"
    fi

    # Overall Proxmox readiness
    if [ "$has_tun" = true ] && [ "$has_nesting" = true ]; then
        print_check "PASS" "Proxmox LXC environment ready for deployment"
    else
        print_check "WARN" "Proxmox LXC requires configuration" "Check TUN device and container privileges"
    fi
}
...
main() {
    ...
    echo -e "${YELLOW}Environment: $([ "$PROXMOX_ENV" = true ] && echo "Proxmox VE" || echo "Standard Linux")${NC}"
    echo

    # Proxmox-specific critical checks first
    if [ "$PROXMOX_ENV" = true ]; then
        print_section "🏥 Proxmox LXC Critical Checks"
        check_proxmox_requirements
    fi
